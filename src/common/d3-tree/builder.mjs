...
class TreeBuilder {
  ...
  _update(source) {
    ...
    nodes.append('foreignObject')
      ...
      .on('click', function (event, d) {
        ...
        if (d.data.isMarriage) {
          opts.callbacks.marriageClick.call(this, event, d.data.extra, d.data.id, d.data.code)
        } else {
          opts.callbacks.nodeClick.call(this, event, d.data.name, d.data.extra, d.data.id, d.data.code, opts)
        }
      })
      .on('contextmenu', function (event, d) {
        ...
        // Após o foreignObject ser inserido, pegue os botões e registre eventos:
        const doc = fo.querySelector('div');
        if (doc) {
          doc.querySelectorAll('.node-button').forEach(btn => {
  ...
  static _nodeSize(nodes, width, minHeight, textRenderer) {
    let maxWidth = 0;
    let maxHeight = 0;
    ...
    _.map(nodes, function (n) {
      ...
      let height = Math.max(minHeight, container.offsetHeight);
